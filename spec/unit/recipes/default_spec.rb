...
ChefSpec.define_matcher :default_spring_service
...
describe 'spring_service::default' do
  context 'When all attributes are default, on an unspecified platform' do
    let(:chef_run) do
      ChefSpec::SoloRunner
        .new(
          step_into: ['spring_service'], \
          cookbook_path: ['./test/cookbooks', '../'], \
          platform: 'centos',
          version: '6.7'
        ) { |node| }
        .converge('spring_service_test::default')
    end
    ...
    it 'creates a symbolic link' do
      expect(chef_run).to create_link('/testdir/test.jar')
